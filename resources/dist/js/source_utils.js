...
/****************************************************************************************/
function alta_saldo_inicial(){
  $(document).ready(function(){
	console.log("TOTAL:"+$("#total").val());
     if($("#fecha").val()!=""||$("#total").val()!=""){
      $.ajax({
         beforeSend: function(){
         ...
         url: 'graba_saldo.php',
         type: 'POST',
         data: 'fecha='+$("#fecha").val()+'&total='+$("#total").val()+'&descipcion='+$("#descipcion").val(),
         success: function(res){
           if(res=='error'){
             var n = noty({
                                   text: "No se registro el saldo inicial, los campos SUBTOTAL, IVA, TOTAL no pueden ser inferiores a Cero...",
                                   theme: 'relax',
                                   layout: 'center',
                                   type: 'error',
                                   timeout: 2000,
                                  });
                                  $("#btn-altas").html("<i class='fa fa-check-circle'></i> Registrar el saldo inicial.");
                                  $("#btn-altas").attr('disabled', false);
           } else{
           var n = noty({
                                   text: "Se registro el saldo inicial correctamente...!",
                                   theme: 'relax',
                                   layout: 'center',
                                   type: 'information',
                                   timeout: 2000,
            ...
            $("#btn-altas").attr('disabled', false);
            $("#btn-altas").html("<i class='fa fa-check-circle'></i> Registrar el saldo inicial.");
            cancela_campos_saldo();
            }
          },
         error: function(jqXHR,estado,error){
           alert("Hubor un error al registrar el saldo inicial...por favor reporte a soporte...!");
