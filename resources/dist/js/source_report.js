...
function sumatorias(){
  $(document).ready(function(){
    ...
    var total_saldoIni=0.00;
	
	/*tabla de gastos*/
    $('#tabla_de_saldo > tbody > tr').each(function(){
                var monto = $(this).find('td').eq(3).html();
                monto=monto.replace(",","");
                monto=parseFloat(monto);
                total_saldoIni+=monto;
         $("#total_saldo_inicial").html("");
         $("#total_saldo_inicial").html("<span class='label label-success'>Total: $ "+total_saldoIni.toFixed(2)+"</span>");
    });
    ...
    $('#tabla_ventas_credito > tbody > tr').each(function(){
       ...
       $("#global").html("<div class='callout callout-success'><h3>Saldo Inicial: $"+total_saldoIni.toFixed(2)+"<br/>Venta: $"+venta_neta.toFixed(2)+"<br>"+
       "Cancelaciones: $"+(detalle_cancelado_contado+detalle_cancelado)+"<br>"+
       "Abonos de Clientes: $"+total_abonos+"<br>"+
       "Gastos: $"+total_gastos+"<br>"+
       "Total en Caja: $"+((total_contado+total_abonos+total_saldoIni-total_gastos).toFixed(2))+"<h3></div>");
     })
}
/*********************************************************************************************/
function print1(){
...
/*******************************************************************************/
 function trae_existencia(){
    $( document ).ready(function(){
        ...
        if(cual=='por_linea'){
          if($("#linea").val()=='0'){
            alert("Selecciona un tipo de producto de la lista...");
            return false;
          } else{
...
function sumatorias_general(){
  $(document).ready(function(){
    ...
    $('#tabla_inventarios > tbody > tr').each(function(){
                ...
                total_ar+=ar;
         $(".aqui").html("");
         $(".aqui").html("<h4>Existencias a la Fecha: "+$("#fecha").val()+" | "+leyenda+" | <span class='label label-primary'>Total de producto: "+total_ar.toFixed(2)+"</span> | <span class='label label-primary'>Costo Total: $ "+total_monto.toFixed(2)+"</label></h4><button type='button' class='btn btn-warning btn-xs pull-right no-print' onclick='print_existe_todo();'><i class='fa fa-print'></i> Imprimir</button>");
        });
     })
}
/**************************************************************************************/
  ...
  $(document).ready(function (){
    if($("#fi").val()!=""||$("#ff").val()!=""){
       $.ajax({
            ...
            type: 'POST',
            data: 'fechai='+$("#fi").val()+'&fechaf='+$("#ff").val()+'&numero_caja='+$("#numero_caja").val(),
            success: function(x){
               $("#data").html(x);
               suma_gastos();
              },
...
/*****************************************************************************/
  function print_saldo_inicial_corte(){
  $(".print20").printArea();
}
/*******************************************************************************/
