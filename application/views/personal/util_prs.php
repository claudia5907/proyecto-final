...
<html>
  <head>
	...
	{
		$("#form_edit_insert_alumno").validate({
			rules: {
				...
				ci:{required: true,
				    remote: {
							url: "<?php echo base_url(); ?>personal/gestion_personal/verificar_ci",
							type: "post",
		    ...
		    submitHandler: function() {
				$("#form_edit_insert_alumno").on("submit", function(event){
					event.preventDefault();
					var parametros=new FormData($(this)[0]);
					 
					$.ajax({
						type: "POST",
						url: "<?php echo base_url(); ?>personal/gestion_personal/insert_update_personal",
						data: parametros, 
						dataType: "html",
	...
	/*******************************************************************************/	
    function eliminar_alumno(id,estado)
	{
		if(estado==0){
			msg='Activar';
		}else{
		...
		swal({
		  title: '¿Estás seguro?',
		  text: "Quieres "+msg+" al Estudiante seleccionado?",
		  type: 'warning',
		  ...
		  cancelButtonText: 'No'
			...
			$.ajax({
			  ...
			  url: '<?php echo base_url()?>personal/gestion_personal/eliminar_personal/'+id+'/'+estado,
			  type: 'POST',
			  data: null,
			  success: function(x){
			   ...
			   error: function(jqXHR,estado,error){
				 $("#personal_registrados").html('Hubo un error: '+estado+' '+error);
				 alert("Hubo un error al consultar estudiantes registrados, contacte a soporte inmediatamente...!");
			   }
			}); 
		}); 
	}	
	function editar_alumno(id)
	{
		$.ajax({
		  beforeSend: function(){ 
				$("#modal_editar_estudiante").html('<i class="fa fa-spinner fa-spin fa-3x fa-fw"></i><span class="sr-only">Espere un momento...</span>');
		   },
		  url: '<?php echo base_url()?>personal/gestion_personal/mostrar_form_editar',
		  type: 'POST',
		  data: {id:id},
		  success: function(x){
				$("#modal_editar_estudiante").html(x);
			 },
		   error: function(jqXHR,estado,error){
				$("#modal_editar_estudiante").html('Hubo un error: '+estado+' '+error);
				alert("Hubo un error al consultar usuarios registrados, contacte a soporte inmediatamente...!");
		   }
		});
	}
	function fn_mostrar_datos_estudiante(id)
	{
		$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>personal/gestion_personal/mostrar_foto_estudiante",
			data: {id:id}, 
			 
			success: function(msg){
				$("#modal_info_datos").html(msg);	 
			}
		}); 
	}
  ...
  <body onload="pone_personal_registrados();">
        <!-- Main content -->
			...
			<div class='col-md-12'> 
				<div class='box box-warning'>
					<div class="box-header with-border">
						<h3 class='miestilo'>Crear Nuevo Estudiante</h3>
					</div>
					<form class="form-horizontal" method="post"   enctype="multipart/form-data" action="#" id="form_edit_insert_alumno" autocomplete="off">
						<div class='box-body'> 
        ...
        <div class="modal fade " id="modal_editar_estudiante" style="overflow-y: scroll;overflow:hidden;" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true"></div>
        <div class="modal fade " id="modal_info_datos" style="overflow-y: scroll;overflow:hidden;" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true"></div>
