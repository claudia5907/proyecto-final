...
<html>
  <head>
	...
	{ 
		$("#form_insert_aula").validate({
			rules: {
				nombre: { required: true },
				descripcion: { required: true} 
			},
			messages: {
				nombre: { required: "<font color='red'>Campo Requerido"},
		    ...
		    submitHandler: function() {
				$("#form_insert_aula").on("submit", function(event){
					event.preventDefault();
					var parametros=new FormData($(this)[0]);
					 
					$.ajax({
						type: "POST",
						url: "<?php echo base_url(); ?>aula/gestion_aula/insert_update_aula",
						data: parametros, 
						dataType: "html",
	...
	/***********************************************************************************/
	function pone_aula_registrados()
	{
	    $.ajax({
		  beforeSend: function(){ 
				$("#aula_registrados").html('<i class="fa fa-spinner fa-spin fa-3x fa-fw"></i><span class="sr-only">Espere un momento...</span>');
		   },
		  url: '<?php echo base_url()?>aula/gestion_aula/pone_aula_regs',
		  type: 'POST',
		  data: null,
		  success: function(x){
				$("#aula_registrados").html(x);
			 },
		   error: function(jqXHR,estado,error){
				$("#aula_registrados").html('Hubo un error: '+estado+' '+error);
    ...
    function eliminar_aula(id)
	{
		swal({
		  title: '¿Estás seguro?',
		  text: "Quieres eliminar el Aula seleccionada?",
		  type: 'warning',
		  ...
		  cancelButtonText: 'No'
			...
			$.ajax({
			  ...
			  url: '<?php echo base_url()?>aula/gestion_aula/eliminar_aula/'+id,
			  type: 'POST',
			  data: null,
			  success: function(x){
			   ...
			   error: function(jqXHR,estado,error){
				 $("#aula_registrados").html('Hubo un error: '+estado+' '+error);
				 alert("Hubo un error al consultar aulas registrados, contacte a soporte inmediatamente...!");
			   }
			}); 
		}); 
	}	
	function editar_aula(id)
	{
		$.ajax({
		  beforeSend: function(){ 
				$("#modal_editar_aula").html('<i class="fa fa-spinner fa-spin fa-3x fa-fw"></i><span class="sr-only">Espere un momento...</span>');
		   },
		  url: '<?php echo base_url()?>aula/gestion_aula/mostrar_form_editar',
		  type: 'POST',
		  data: {id:id},
		  success: function(x){
				$("#modal_editar_aula").html(x);
			 },
		   error: function(jqXHR,estado,error){
				$("#modal_editar_aula").html('Hubo un error: '+estado+' '+error);
				alert("Hubo un error al consultar materias registrados, contacte a soporte inmediatamente...!");
		   }
		});
	}
  ...
  <body onload="pone_aula_registrados();">
        <!-- Main content -->
        ...
        <div class='row'>
			<div class='col-md-12'> 
				<div class='box box-warning'>
					<div class="box-header with-border">
						<h3 class='miestilo'>Crear Nueva Aula</h3>
					</div>
					<form class="form-horizontal" method="post"   enctype="multipart/form-data" action="#" id="form_insert_aula" autocomplete="off">
						<div class='box-body'> 
			...
			<div class='col-md-12'>
				<div id='aula_registrados'></div>
			</div>
        </div>
        </section>
        <div class="modal fade " id="modal_editar_aula" style="overflow-y: scroll;overflow:hidden;" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true"></div>
